...
require(ggplot2)
niter<-1000
example<-res_trend(dataset,niter,ref_year=2001,correction = TRUE)
...
ggplot(dataset, aes(x = Year, y = Index))+
  geom_ribbon(aes(ymin = Index-Index_SE, ymax = Index+Index_SE), fill = "lightgrey")+
    ...
    stat_function(fun = function(x){example$alpha2*x^2+example$alpha1*x+example$inter-1.96*example$strd/sqrt(niter)},linetype="dashed")+
    stat_function(fun = function(x){example$alpha2*x^2+example$alpha1*x+example$inter+1.96*example$strd/sqrt(niter)},linetype="dashed")+
    ...
    geom_segment(x=example$p_1+1.96*example$sd_p_1,y=0,aes(xend=example$p_1+1.96*example$sd_p_1/sqrt(niter),yend=min(abs(dataset$Index))),color="red", linetype="dashed")+
    geom_segment(x=example$p_1-1.96*example$sd_p_1,y=0,aes(xend=example$p_1-1.96*example$sd_p_1/sqrt(niter),yend=min(abs(dataset$Index))),color="red", linetype="dashed")+
    ...
    geom_segment(x=example$p_2+1.96*example$sd_p_2,y=0,aes(xend=example$p_2+1.96*example$sd_p_2/sqrt(niter),yend=min(abs(dataset$Index))),color="blue", linetype="dashed")+
    geom_segment(x=example$p_2-1.96*example$sd_p_2,y=0,aes(xend=example$p_2-1.96*example$sd_p_2/sqrt(niter),yend=min(abs(dataset$Index))),color="blue", linetype="dashed")+
    ...
    geom_segment(x=example$p_3+1.96*example$sd_p_3,y=0,aes(xend=example$p_3+1.96*example$sd_p_3/sqrt(niter),yend=min(abs(dataset$Index))), color="blue", linetype="dashed")+
    geom_segment(x=example$p_3-1.96*example$sd_p_3, y=0,aes(xend=example$p_3-1.96*example$sd_p_3/sqrt(niter), yend=min(abs(dataset$Index))),color="blue", linetype="dashed")
```
